...
# Get machine name (CHatGPT)
machine_name = socket.gethostname()

# Get operating system name (CHatGPT)
os_name = platform.system()

# Get operating system version (CHatGPT)
os_version = platform.release()

# Get number of CPU's (CHatGPT)
num_cpus = os.cpu_count()

# Get amount of memory (CHatGPT)
memory = round(psutil.virtual_memory().total / (1024.0 ** 3),2)

# Get IP address (CHatGPT)
ip_address = socket.gethostbyname(socket.gethostname())

# Print the #1 to 6 information (CHatGPT)
...
# For #7 information using the sample code 1 at page 5 of the practical slides (COMP30830 Practical 01_01)
import timeit 
import math 
import itertools
...
if __name__ == '__main__':
	...
	print('7. CPU strength score(relative elapsed):', round(elapsed_time/t_default,3))
print()